...
<form ng-submit="app.reqLite('/pages/mypages?q='+pagesearch)">
 <div class="row">
...
if( !mysql_num_rows($result) ) 
  echo "<br><br>Search Results Not found";
    ...
    if( $limit_f > 0 ) 
        echo" <a href='#/pages/mypages?q=".$_GET['q']."&f=$new_limit_f'> | Previous</a> ";
    else 
        echo" | Previous";
    if( $show_next ) 
        echo" <a href='#/pages/mypages?q=".$_GET['q']."&f=$limit_t'> | Next</a> ";
    else 
      echo" | Next";
    ...
    while($row = mysql_fetch_object($result) ){
        ...
        echo"<a href='#/pages/trash?q=".$row->id."' class='button tiny right'> Move to trash</a>";
        echo"<a href='#/pages/edit?q=".$row->id."' class='button tiny right'> Edit</a>";
        if( $row->staged_changes ) 
          echo "<a id = 'publish".$row->id."' ng-click=\"app.request({ location:'/pages/publish?q=".$row->id."'} )\" class='button tiny right'> Publish </a>
                <a href='#/pages/discard?q=".$row->id."' class='button tiny right'> Discard </a>";
